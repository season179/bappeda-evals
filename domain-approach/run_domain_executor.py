#!/usr/bin/env python3
"""
Domain RAG Executor for Evaluation
Runs test queries from testset.jsonl against SmartKnowledge RAG app
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Execute RAG queries for domain evaluation",
    ...
    parser.add_argument(
        ...
        type=str,
        default="config.yaml",
        help="Path to configuration file (default: config.yaml in parent dir)"
    )
    ...
    parser.add_argument(
        ...
        help="Input test dataset JSONL file (default: testset.jsonl)"
    )
...
def main():
    ...
    logger = setup_logger_from_config(
        name="domain_executor",
        config=config,
        verbose=args.verbose,
        title="Domain RAG Executor for Evaluation"
    )
    ...
    timestamp = None
    checkpoint_pattern_str = 'domain_executor_checkpoint_*.json'

    if checkpoint_enabled and args.resume:
        # Find most recent checkpoint file
        checkpoint_pattern = Path(__file__).parent.glob(checkpoint_pattern_str)
        checkpoint_files = sorted(checkpoint_pattern, reverse=True)
        if checkpoint_files:
            latest_checkpoint = checkpoint_files[0]
            # Extract timestamp from filename: domain_executor_checkpoint_YYMMDD_HHMMSS.json
            timestamp = latest_checkpoint.stem.replace('domain_executor_checkpoint_', '')
    ...
    # Construct timestamped filenames (in domain-approach directory)
    output_file = f"domain_execution_{timestamp}.jsonl"
    checkpoint_file = f"domain_executor_checkpoint_{timestamp}.json"
    ...
    try:
        df = load_test_dataset(str(input_path), limit=args.limit)
    ...
    for idx in range(start_index, total_queries):
        ...
        # Use query_id from file if available, otherwise use index
        # Convert to native Python int to ensure JSON serialization works
        query_id = int(row.get('query_id', idx))
    ...
    logger.info("\n✓ Domain RAG Executor completed successfully!")
